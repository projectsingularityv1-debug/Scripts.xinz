...
do
	...
	-- Load local lucide spritesheet icon engine (lucide.lua)
	local _lucidePath = script and script.Parent and script.Parent:FindFirstChild("lucide") or nil
	local IconList
	if _lucidePath then
		IconList = require(_lucidePath)
	else
		-- Fallback: load from readfile if available (exploit environment)
		local ok, result = pcall(function()
			local src = readfile and readfile("K2NTA ST/Scripts/UI.main/lucide.lua") or nil
			if src then return loadstring(src)() end
		end)
